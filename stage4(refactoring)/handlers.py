...
async def sendMENU(update_or_query, context):
    ...
    Shows admin‑specific buttons for main_admin and regular admins.
    """
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    /start handler – registers the user and shows the main menu.
    """
...
async def buttonreceived(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "meetings":
        ...
        # Show list of meetings with participant statuses, weather, and AI advice button
        ...
        for meeting_id in rows:
            ...
            for row in meetings:
                ...
                text = ""
                text += f"Дата: {format_datetime_moscow(row[1], row[2])} (МСК)\n"
                text += f"Место: {row[3]}\n"
                ...
                # Get weather forecast if city is set
                ...
                # AI advice button (only if city is set)
        ...
        # Delete a meeting (admin or only participant)
        ...
        if status == "accepted":
            # Notify all accepted participants that the user declined
            for usid in rows[0]:
            ...
            # Notify all accepted participants that the user accepted
            for usid in rows[0]:
        ...
        # Handle acceptance/declination of an invitation
        parts = data.split("_")
        user_id = int(parts[2])
        meeting_id = int(parts[1])
        ...
        # AI advice based on weather and place
        parts = data.split("_")
        if len(parts) < 3:
...
async def adminreceived(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if db.user_exists(target_id):
        ...
        if db.check_admin(target_id):
            ...
            await context.bot.send_message(chat_id=target_id, text="Права администратора изъяты", reply_markup=reply_markup)
        else:
            db.give_or_revoke_admin(target_id, 1)
            await update.message.reply_text('Выданы права администратора', reply_markup=reply_markup)
            reply_markup = InlineKeyboardMarkup(keyboardm)
            await context.bot.send_message(chat_id=target_id, text="Выданы права администратора", reply_markup=reply_markup)
    else:
        keyboard = [[menu_btn]]
        reply_markup = InlineKeyboardMarkup(keyboard)
        await context.bot.send_message(chat_id=target_id, text="Пользователь не найден, проверьте ID!", reply_markup=reply_markup)

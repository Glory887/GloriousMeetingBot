...
async def sendMENU(update_or_query, context):
    ...
    Uses the user's language to localize button labels.
    Shows additional admin buttons for main_admin and regular admins.
    """
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    /start handler – registers the user if new, then shows the main menu.
    """
...
async def buttonreceived(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    Dispatches to the appropriate action based on callback_data.
    """
    ...
    if data == "meetings":
        ...
        for meeting_id in rows:
            ...
            for row in meetings:
                ...
                # Select date format based on language (MSK for RU, UTC for EN)
                if lang == 'en':
...
async def datereceived(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Receive date, validate format, store in context, and ask for time.
    """
    user_id = update.effective_user.id
    lang = get_user_lang(user_id)
...
async def timereceived(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Receive time, validate format and ensure it's in the future.
    Convert user input (MSK) to UTC, store it, show weather if city is set,
    then ask for place.
    """
    user_id = update.effective_user.id
    lang = get_user_lang(user_id)
...
async def placereceived(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Receive place, store it, and ask for comment.
    """
    user_id = update.effective_user.id
    lang = get_user_lang(user_id)
...
async def invitee_received(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    Send a private message to the invited user with accept/decline buttons.
    """
    query = update.callback_query
    await query.answer()
    data = query.data
...
async def cancel_conversation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Cancel the current dialog with /cancel command.
    """
    user_id = update.effective_user.id
    lang = get_user_lang(user_id)
    keyboard = [[InlineKeyboardButton(get_text(lang, 'menu_btn'), callback_data="menu")]]
...
async def adminreceived(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Grant or revoke admin rights for a given user ID.
    """
    user_id = update.effective_user.id
    lang = get_user_lang(user_id)
    try:
...
async def adminlistreceived(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Show the list of current administrators.
    """
    query = update.callback_query
    await query.answer()
    user_id = query.from_user.id
...
async def cityreceived(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Set the user's city for weather forecasts.
    """
    user_id = update.effective_user.id
    lang = get_user_lang(user_id)
